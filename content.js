...
async function addProfileToFeedServer(feedId, profileData) {
  try {
    ...
    chrome.storage.local.get(["feeds"], (data) => {
      ...
      // Initialiser le tableau des profils s'il n'existe pas
      if (!data.feeds[feedIndex].profiles) {
        data.feeds[feedIndex].profiles = []
      }

      // Ajouter le nouveau profil
      data.feeds[feedIndex].profiles.push({
        id: newProfile.id,
        uniqueId: newProfile.unique_id,
        name: newProfile.name,
        title: newProfile.title || "",
        imageUrl: newProfile.image_url || "",
        linkedinUrl: newProfile.linkedin_url,
      })
...
function addProfileButtonFn() {
  if (!isProfilePage() || window.location.pathname === "/feed/") {
    console.log(\"Not on a profile page or on feed page, skipping
